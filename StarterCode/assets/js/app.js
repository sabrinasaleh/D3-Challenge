...
// Note: Making this a function allows to easily call it in the mobility section of the code.
var circRadius;
function crGet() {
  if (width <= 530) {
    circRadius = 5;
...
// Section 2: The Labels for Axes
// ===========================

// A) Bottom Axis
// Create a group element to nest the bottom axes labels
svg.append("g").attr("class", "xText");
// xText allows to select the group without excess code
var xText = d3.select(".xText");
// Give xText a transform property that places it at the bottom of the chart.
// Nesting this attribute in a function allows to change the location of the label group whenever the width of the window changes.
...
// B) Left Axis
// Specify the variables the following way allows to make the transform attributes more readable
var leftTextX = margin + tPadLeft;
var leftTextY = (height + labelArea) / 2 - labelArea;
// Add a second label group, this time for the axis left of the chart
svg.append("g").attr("class", "yText");
// yText allows to select the group without excess code
var yText = d3.select(".yText");
// Like before, nesting the group's transform attr in a function allows to change the location of the label group whenever the width of the window changes.
...
// Section 3: Import the csv file
// ========================

// This data file includes state-by-state demographic data from the US Census and measurements from health risks obtained by the Behavioral Risk Factor Surveillance System.
...
// Section 4: Create visualization function
// ====================================

// Define a "visualize" function on the data obtained with d3's .csv method.
// This function handles the visual manipulation of all elements dependent on the data.
function visualize(theData) {
  // Part I: Essential local variables and functions
  // =================================
  ...
  var toolTip = d3
    ...
    .html(function(d) {
      ...
      // Extract the y value's key and value
      ...
      // Display what has been captured
      return theState + theX + theY;
    });
  // Call the toolTip function.
  svg.call(toolTip);
  // Part II: Functions to prevent repitition
  // ==============  
  ...
  // Part III: Instantiate the Scatter Plot
  // ====================================
  // This will add the first placement of the data and axes to the scatter plot.
  // First grab the min and max values of x and y.
  xMinMax();
  yMinMax();
  // With the min and max values defined, now scales can be created.
  // Including the margin and word area in the range method tells d3 to place the circles in an area starting after the margin and word area.
  var xScale = d3
    .scaleLinear()
    .domain([xMin, xMax])
  ...
  var yScale = d3
    .scaleLinear()
    .domain([yMin, yMax])
    // Height is inverses due to d3 calc's y-axis placement
  ...
  // Call them to include all of the numbers, borders and ticks.
  ...
  // Make a grouping for the dots and their labels
  ...
  // With the circles on the  graph, matching labels are required.
  // Grab the state abbreviations from the data and place them in the center of the dots.
  theCircles
    .append("text")
    ...
    // Place the text using the created scale
    .attr("dx", function(d) {
      return xScale(d[curX]);
    })
    .attr("dy", function(d) {
  ...
  // Part IV: Make the graph dynamic
  // ==========================
  ...
  d3.selectAll(".aText").on("click", function() {
    ...
    // Only run this on inactive labels    
    if (self.classed("inactive")) {
      ...
      if (axis === "x") {
        ...
        // Now use a transition for the update of the xAxis
        ...
        d3.selectAll("circle").each(function() {
          ...
          // This will lend the circle a motion tween from it's original spot to the new location.
          d3
            .select(this)
            .transition()
            .attr("cx", function(d) {
  ...
  // Part V: Mobile Responsive
  // =========================
  // Call a resize function whenever the window dimensions change.
  // This make's it possible to add true mobile-responsiveness to the charts.
